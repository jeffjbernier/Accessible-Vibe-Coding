...
- Text must have a minimum contrast ratio of 4.5:1 against its background. Large text (18pt/24px+ regular, or 14pt/~18.66px+ bold) requires 3:1.
...
- Target areas for pointer interactions must be at least 44×44 CSS pixels. This ruleset builds to the AAA size (WCAG SC 2.5.5) rather than the AA minimum of 24×24 (SC 2.5.8).
...
1. **Automated scanning**: Run axe-core, Lighthouse Accessibility, or WAVE on every page. If the `accessibility-rules` skill is installed, its `scripts/axe-check.js` drives axe-core over a URL or a local HTML file.
